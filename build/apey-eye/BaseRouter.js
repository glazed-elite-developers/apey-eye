...
var BaseRouter = (function () {
    ...
    _createClass(BaseRouter, [{
        ...
        value: function register(entries) {
            ...
            if (entries) {
                entries.forEach(function (entry) {
                    if (entry.resource && entry.resource.prototype instanceof _ResourceJs2['default']) {
        ...
        value: function rootOptions(baseUri) {
            var options = {},
                resources = {},
                basePath = _configRouterJs2['default'].basePath || '/';

            Object.keys(this.entries).forEach(function (entry) {
                resources[entry] = '' + baseUri + '' + _configRouterJs2['default'].basePath + '/' + entry;
            });
            ...
            if (!allowedRoles) {
                ...
                if (allowedRoles.length === 0) {
                    ...
                    if (allowedRoles.indexOf(user.role) > -1) {
                        ...
                        var childRoles = yield _modelsRoleModelJs2['default'].fetch({
                            resourceProperties: {
                                _filter: {
                        ...
                        while (childRoles.length > 0) {
                            ...
                            if (allowedRoles.indexOf(role.id) > -1) {
                                ...
                                var newChilds = yield _modelsRoleModelJs2['default'].fetch({
                                    resourceProperties: {
                                        _filter: {
                                            parentRole: role.id
                                        },
        ...
        key: 'resourceName',
        value: function resourceName(resourcePath) {
